    ...
    const fetchStoreItems = async () => {
      try {
        ...
        if (res.status >= 500) {
          ...
          setItems([]);
          setError(
            `Sunucu şu anda kullanılamıyor. Örnek veriler gösteriliyor. (${res.status}).`
          );
          setLoading(false);
          return;
        }
        ...
        if (!res.ok) {
          throw new Error(
            `Failed to fetch store items: ${res.status} ${res.statusText}`
          );
        }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="mt-2 text-gray-600">
              Puanlarınızı harika ödüllerle değerlendirin!
            </p>
          </div>
          <Link
        ...
        <div className="bg-white shadow-lg rounded-2xl p-6 mb-8 border border-gray-100">
          ...
          <p className="mt-4 text-gray-600 bg-gray-50 p-3 rounded-lg border border-gray-100">
            Mağazadan ürün talep etmek için puanlarınızı kullanabilirsiniz. Tüm
            talepler öğretmeniniz tarafından onaylanmalıdır.
          </p>
        </div>
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-xl mb-8 flex items-center gap-3">
            ...
            {error.includes("Database connection issue")
              ? "Veritabanı bağlantı sorunu. Örnek veriler gösteriliyor."
              : error.includes("Server currently unavailable")
              ? "Sunucu şu anda kullanılamıyor. Örnek veriler gösteriliyor."
              : error.includes("Server connection timed out")
              ? "Sunucu bağlantı zaman aşımı. Örnek veriler gösteriliyor."
              : error.includes("Database or server issue")
        ...
        {successMessage && (
          <div className="bg-green-50 border border-green-200 text-green-700 px-6 py-4 rounded-xl mb-8 flex items-center gap-3">
            ...
            {successMessage.includes("has been submitted successfully")
              ? successMessage
                  .replace("Your request for", "Talebiniz")
                  .replace(
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 max-w-lg w-full">
            ...
            <p className="text-gray-600 mb-6">
              ...
              için talep oluşturmak üzeresiniz. Bu işlem{" "}
              <span className="font-medium text-gray-900">
